...
export class ExamplePlatformAccessory {
  ...
  async setOn(value: CharacteristicValue) {
    if (value as boolean){
      const url = this.platform.config['url'] + '&SetR=0' + this.accessory.context.device.index;
      axios.get(url);
      this.exampleStates.On = true;
    } else {
      const url = this.platform.config['url'] + '&ClearR=0' + this.accessory.context.device.index;
      axios.get(url);
